...
export const getCourseProgress = createAction({
  ...
  execute: async (input, context) => {
    try {
      const progress = await studentRepository.getCourseProgress(context.userId, input.courseId)
...
export const getAvailableCourses = createAction({
  ...
  execute: async (input, context) => {
    try {
      ...
      const courses = await studentRepository.getAvailableCourses(context.userId, limit, skip)
...
export const checkEnrollment = createAction({
  ...
  execute: async (input, context) => {
    try {
      const enrollment = await studentRepository.getEnrollmentByCourseId(input.courseId, context.userId)
