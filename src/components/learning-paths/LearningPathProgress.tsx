...
export function LearningPathProgress({ nodes, completedNodes, currentNodeId }: LearningPathProgressProps) {
          ...
          {nodes.map((node, index) => {
            ...
            return (
              <div key={node.id} className='flex items-center gap-4'>
                <div className='flex-shrink-0'>{getNodeIcon(status)}</div>
                ...
                {index < nodes.length - 1 && <div className='absolute left-[22px] w-0.5 h-8 bg-gray-300 -ml-6' />}
              </div>
            )
          })}
        </div>
