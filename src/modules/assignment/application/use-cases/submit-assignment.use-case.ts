...
export class SubmitAssignmentUseCase extends BaseUseCase<SubmitAssignmentRequest, SubmissionResponse> {
  ...
  async execute(request: SubmitAssignmentRequest): Promise<Result<SubmissionResponse>> {
    ...
    try {
      ...
      return Result.fail(new Error(`Failed to submit assignment: ${(error as Error).message}`))
    }
  }
}
