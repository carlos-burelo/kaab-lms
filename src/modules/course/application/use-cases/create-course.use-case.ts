...
import { DuplicateEntityError, ForbiddenError } from '@/core/shared/errors'
...
import { CoursePrice, CourseSlug, CourseTitle } from '../../domain/value-objects'
...
export class CreateCourseUseCase extends BaseUseCase<CreateCourseRequest, CourseDTO> {
  ...
  async execute(request: CreateCourseRequest): Promise<Result<CourseDTO>> {
    ...
    if (dto.instructorId !== currentUserId) {
      return Result.fail(new ForbiddenError('You can only create courses for yourself'))
    }
    ...
    const slugExistsResult = await this.courseRepository.slugExists(slug.value)
    if (slugExistsResult.isFailure) {
      return Result.fail(slugExistsResult.error)
    }

    if (slugExistsResult.value) {
      return Result.fail(new DuplicateEntityError('Course', 'slug', slug.value))
    }
    ...
    // Save to repository
    const savedResult = await this.courseRepository.save(courseResult.value)
