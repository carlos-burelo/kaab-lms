...
export class GetCourseUseCase extends BaseUseCase<GetCourseRequest, CourseDTO> {
  ...
  async execute(request: GetCourseRequest): Promise<Result<CourseDTO>> {
    ...
    if (!courseId && !slug) {
      return Result.fail(new EntityNotFoundError('Course', 'id or slug must be provided'))
    }

    // Find course by ID or slug
    const courseResult = slug ? await this.courseRepository.findBySlug(slug) : await this.courseRepository.findById(courseId!)
    ...
    if (!courseResult.value) {
      return Result.fail(new EntityNotFoundError('Course', courseId || slug!))
    }
