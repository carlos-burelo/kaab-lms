...
export class PublishCourseUseCase extends BaseUseCase<
...
> {
  ...
  async execute(
    ...
    if (!courseResult.value) {
      return Result.fail(
        new EntityNotFoundError('Course', courseId)
      );
    }
    ...
    if (!course.canBeEditedBy(currentUserId)) {
      return Result.fail(
        new ForbiddenError('You do not have permission to publish this course')
      );
    }
