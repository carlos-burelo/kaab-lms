...
import { CoursePrice, CourseTitle } from '../../domain/value-objects'
...
export class UpdateCourseUseCase extends BaseUseCase<UpdateCourseRequest, CourseDTO> {
  constructor(private courseRepository: ICourseRepository) {
    super()
  }

  async execute(request: UpdateCourseRequest): Promise<Result<CourseDTO>> {
    ...
    if (!courseResult.value) {
      return Result.fail(new EntityNotFoundError('Course', dto.courseId))
    }
    ...
    if (!course.canBeEditedBy(currentUserId)) {
      return Result.fail(new ForbiddenError('You do not have permission to update this course'))
    }
    ...
    if (dto.discountPrice !== undefined) {
      ...
      const applyDiscountResult = course.applyDiscount(discountPriceResult.value)
