...
export class GetEnrollmentUseCase extends BaseUseCase<GetEnrollmentRequest, EnrollmentDTO> {
  constructor(private enrollmentRepository: IEnrollmentRepository) {
    super()
  }

  async execute(request: GetEnrollmentRequest): Promise<Result<EnrollmentDTO>> {
    ...
    if (enrollmentId) {
      ...
      enrollmentResult = await this.enrollmentRepository.findByUserAndCourse(userId, courseId)
    } else {
      return Result.fail(new NotFoundError('Enrollment', 'query', 'Either enrollmentId or userId+courseId must be provided'))
    }
    ...
    if (!enrollmentResult.value) {
      return Result.fail(
        new NotFoundError('Enrollment', enrollmentId ? 'id' : 'userId-courseId', enrollmentId || `${userId}-${courseId}`)
      )
    }
