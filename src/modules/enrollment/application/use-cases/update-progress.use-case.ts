...
export class UpdateProgressUseCase extends BaseUseCase<
...
> {
  ...
  async execute(
    ...
    // Find enrollment
    const enrollmentResult = await this.enrollmentRepository.findById(
      dto.enrollmentId
    );
    ...
    if (!enrollmentResult.value) {
      return Result.fail(
        new NotFoundError('Enrollment', 'id', dto.enrollmentId)
      );
    }
    ...
    // Update progress
    const updateResult = enrollment.updateProgress(
    ...
    if (dto.lastLessonId && dto.timeSpentMinutes !== undefined) {
      const completeLessonResult = enrollment.completeLesson(
        dto.lastLessonId,
