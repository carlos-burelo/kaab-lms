...
import { CourseCompletedEvent, ProgressUpdatedEvent, StudentEnrolledEvent } from './events'
...
export class Enrollment extends AggregateRoot<EnrollmentProps> {
   ...
   */
  static create(
    props: Omit<EnrollmentProps, 'id' | 'progress' | 'totalTimeMinutes' | 'isCompleted' | 'createdAt' | 'updatedAt'>
    ...
    if (!props.courseId || props.courseId.trim().length === 0) {
      return Result.fail(new ValidationError('Course ID is required', 'courseId'))
    }
   ...
   * Update progress
   */
  updateProgress(completedLessons: number, totalLessons: number): Result<void, ValidationError> {
    if (completedLessons < 0 || totalLessons <= 0) {
      return Result.fail(new ValidationError('Invalid lesson counts', 'progress'))
    }

    if (completedLessons > totalLessons) {
      return Result.fail(new ValidationError('Completed lessons cannot exceed total lessons', 'progress'))
    }
   ...
   * Complete a lesson
   */
  completeLesson(lessonId: string, timeSpentMinutes: number): Result<void, ValidationError> {
    if (!lessonId || lessonId.trim().length === 0) {
      return Result.fail(new ValidationError('Lesson ID is required', 'lessonId'))
    }

    if (timeSpentMinutes < 0) {
      return Result.fail(new ValidationError('Time spent cannot be negative', 'timeSpentMinutes'))
    }
  ...
  complete(): Result<void, BusinessRuleError> {
    if (this._props.isCompleted) {
      return Result.fail(new BusinessRuleError('Enrollment is already completed'))
    }
