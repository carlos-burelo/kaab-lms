...
export class UpdateFileUseCase extends BaseUseCase<UpdateFileRequest, FileDTO> {
  ...
  async execute(request: UpdateFileRequest): Promise<Result<FileDTO>> {
    ...
    if (file.uploadedBy !== currentUserId) {
      return Result.fail(new ForbiddenError('You do not have permission to update this file'))
    }
