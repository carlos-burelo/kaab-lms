...
import { z } from 'zod'
import { MissionDifficultyValues, MissionTypeValues } from '../../domain/value-objects'

export const UpdateMissionSchema = z
  .object({
    title: z.string().min(3, 'Mission title must be at least 3 characters').optional(),
    description: z.string().min(10, 'Mission description must be at least 10 characters').optional(),
    type: z.enum(MissionTypeValues as [string, ...string[]]).optional(),
    difficulty: z.enum(MissionDifficultyValues as [string, ...string[]]).optional(),
