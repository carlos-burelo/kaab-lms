...
export class AwardBadgeUseCase extends BaseUseCase<AwardBadgeRequest, void> {
  ...
  async execute(request: AwardBadgeRequest): Promise<Result<void>> {
    ...
    const hasBadgeResult = await this.badgeRepository.userHasBadge(dto.userId, dto.badgeId)
    ...
    if (hasBadgeResult.value) {
      return Result.fail(new DuplicateEntityError('User already has this badge', 'badgeId', dto.badgeId))
    }
    ...
    const awardToUserResult = await this.badgeRepository.awardBadgeToUser(dto.userId, dto.badgeId)
