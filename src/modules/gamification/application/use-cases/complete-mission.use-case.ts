...
export class CompleteMissionUseCase extends BaseUseCase<CompleteMissionRequest, void> {
  constructor(
  ...
  async execute(request: CompleteMissionRequest): Promise<Result<void>> {
    ...
    const hasCompletedResult = await this.missionRepository.userHasCompletedMission(dto.userId, dto.missionId)
    ...
    if (hasCompletedResult.value) {
      return Result.fail(new DuplicateEntityError('User has already completed this mission', 'missionId', dto.missionId))
    }
    ...
    const completeMissionResult = await this.missionRepository.completeMissionForUser(dto.userId, dto.missionId)
    ...
    if (mission.xpReward > 0) {
      const addXpResult = userGamification.addXp(mission.xpReward, `Mission completed: ${mission.title}`)
    ...
    if (mission.coinReward > 0) {
      const addCoinsResult = userGamification.addCoins(mission.coinReward, `Mission completed: ${mission.title}`)
