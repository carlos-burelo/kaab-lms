...
export class GetActiveMissionsUseCase extends BaseUseCase<GetActiveMissionsRequest, MissionDTO[]> {
  constructor(private missionRepository: IMissionRepository) {
    super()
  }

  async execute(_request: GetActiveMissionsRequest): Promise<Result<MissionDTO[]>> {
    // Get active missions
    ...
    // Map to DTOs
    const missionDTOs = missionsResult.value.map((mission) => missionMapper.toDTO(mission))
