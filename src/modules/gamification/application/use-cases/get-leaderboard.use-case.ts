...
export class GetLeaderboardUseCase extends BaseUseCase<GetLeaderboardRequest, LeaderboardEntryDTO[]> {
  constructor(private userGamificationRepository: IUserGamificationRepository) {
    super()
  }

  async execute(request: GetLeaderboardRequest): Promise<Result<LeaderboardEntryDTO[]>> {
    const { limit = 10, offset = 0 } = request

    // Get leaderboard
    const leaderboardResult = await this.userGamificationRepository.getLeaderboard(limit, offset)
