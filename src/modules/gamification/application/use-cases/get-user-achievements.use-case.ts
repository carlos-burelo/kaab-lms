...
export class GetUserAchievementsUseCase extends BaseUseCase<GetUserAchievementsRequest, UserAchievementDTO[]> {
  constructor(private achievementRepository: IAchievementRepository) {
    super()
  }

  async execute(request: GetUserAchievementsRequest): Promise<Result<UserAchievementDTO[]>> {
    const { userId } = request

    // Get user achievements
    const achievementsResult = await this.achievementRepository.getUserAchievements(userId)
    ...
    const achievementDTOs = achievementsResult.value.map((item) => ({
      ...
      progressPercentage: item.achievement.calculateProgressPercentage(item.progress),
