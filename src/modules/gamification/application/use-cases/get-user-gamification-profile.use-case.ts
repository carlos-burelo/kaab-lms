...
export class GetUserGamificationProfileUseCase extends BaseUseCase<GetUserGamificationProfileRequest, UserGamificationDTO> {
  constructor(private userGamificationRepository: IUserGamificationRepository) {
    super()
  }

  async execute(request: GetUserGamificationProfileRequest): Promise<Result<UserGamificationDTO>> {
    const { userId } = request

    // Get or create user gamification profile
    const userGamificationResult = await this.userGamificationRepository.getOrCreate(userId)
    ...
    // Map to DTO
    const userGamificationDTO = userGamificationMapper.toDTO(userGamificationResult.value)
