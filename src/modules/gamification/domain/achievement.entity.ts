...
export class Achievement extends AggregateRoot<AchievementProps> {
   ...
   */
  static create(props: Omit<AchievementProps, 'id' | 'createdAt' | 'updatedAt'>): Result<Achievement, ValidationError> {
    // Validations
    if (props.name.trim().length < 3) {
      return Result.fail(new ValidationError('Achievement name must be at least 3 characters', 'name'))
    }

    if (props.description.trim().length < 10) {
      return Result.fail(new ValidationError('Achievement description must be at least 10 characters', 'description'))
    }
    ...
    if (props.maxProgress < 1) {
      return Result.fail(new ValidationError('Max progress must be at least 1', 'maxProgress'))
    }
