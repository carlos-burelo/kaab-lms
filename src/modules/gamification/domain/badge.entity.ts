...
export class Badge extends AggregateRoot<BadgeProps> {
   ...
   */
  static create(
    props: Omit<BadgeProps, 'id' | 'createdAt' | 'updatedAt'>
  ): Result<Badge, ValidationError> {
    // Validations
    if (props.name.trim().length < 3) {
      return Result.fail(
        new ValidationError('Badge name must be at least 3 characters', 'name')
      );
    }
    ...
    if (props.description.trim().length < 10) {
      return Result.fail(
        new ValidationError(
          'Badge description must be at least 10 characters',
          'description'
        )
      );
    }
