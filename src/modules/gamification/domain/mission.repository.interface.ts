...
export interface IMissionRepository extends Repository<Mission> {
   ...
   */
  userHasCompletedMission(userId: string, missionId: string): Promise<Result<boolean>>

  /**
   * Mark mission as completed for user
   */
  completeMissionForUser(userId: string, missionId: string): Promise<Result<void>>

  /**
   * Get user mission progress
   */
  getUserMissionProgress(userId: string, missionId: string): Promise<Result<{ progress: number; completedAt?: Date } | null>>
}
