...
import { CoinsAddedEvent, LevelUpEvent, XpAddedEvent } from './events'
...
export class UserGamification extends AggregateRoot<UserGamificationProps> {
   ...
   */
  static create(
    props: Omit<UserGamificationProps, 'id' | 'xp' | 'level' | 'coins' | 'totalBadges' | 'streak' | 'createdAt' | 'updatedAt'>
  ...
  static calculateXpForLevel(level: number): number {
    ...
    return Math.floor(UserGamification.XP_PER_LEVEL_BASE * UserGamification.XP_GROWTH_FACTOR ** (level - 1))
  }
  ...
  addXp(amount: number, reason: string): Result<void, ValidationError> {
    if (amount <= 0) {
      return Result.fail(new ValidationError('XP amount must be positive', 'amount'))
    }
  ...
  addCoins(amount: number, reason: string): Result<void, ValidationError> {
    if (amount <= 0) {
      return Result.fail(new ValidationError('Coin amount must be positive', 'amount'))
    }
  ...
  spendCoins(amount: number): Result<void, BusinessRuleError> {
    if (amount <= 0) {
      return Result.fail(new BusinessRuleError('Coin amount must be positive'))
    }

    if (this.coins < amount) {
      return Result.fail(new BusinessRuleError('Insufficient coins'))
    }
  ...
  private checkAndApplyLevelUp(): number {
    ...
    while (true) {
      const xpForNextLevel = UserGamification.calculateXpForLevel(this.level + 1)
