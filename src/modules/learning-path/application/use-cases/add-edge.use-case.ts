...
import { type LearningPathEdgeDTO, learningPathEdgeMapper } from '../../infrastructure/learning-path.mapper'
import type { AddEdgeDTO } from '../dtos'
...
export class AddEdgeUseCase extends BaseUseCase<AddEdgeRequest, LearningPathEdgeDTO> {
  ...
  async execute(request: AddEdgeRequest): Promise<Result<LearningPathEdgeDTO>> {
    ...
    const learningPathResult = await this.learningPathRepository.findByIdWithGraph(dto.learningPathId)
