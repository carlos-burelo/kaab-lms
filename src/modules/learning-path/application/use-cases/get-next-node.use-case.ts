...
export class GetNextNodeUseCase extends BaseUseCase<
  GetNextNodeRequest,
  LearningPathNodeDTO | null
> {
  ...
  async execute(
    request: GetNextNodeRequest
  ): Promise<Result<LearningPathNodeDTO | null>> {
    ...
    if (!currentNodeId) {
      return Result.fail(
        new BusinessRuleError('No current node in progress')
      );
    }
    ...
    const outgoingEdges = learningPath.edges.filter(
      (edge) => edge.sourceNodeId === currentNodeId
    );

    if (outgoingEdges.length === 0) {
      return Result.fail(
        new BusinessRuleError('No outgoing edges from current node')
      );
    }
