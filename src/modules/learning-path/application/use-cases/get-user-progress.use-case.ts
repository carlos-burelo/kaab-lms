...
export class GetUserProgressUseCase extends BaseUseCase<GetUserProgressRequest, UserLearningPathProgressDTO> {
  constructor(private learningPathRepository: ILearningPathRepository) {
    super()
  }

  async execute(request: GetUserProgressRequest): Promise<Result<UserLearningPathProgressDTO>> {
    ...
    // Map to DTO
    const progressDTO = userLearningPathProgressMapper.toDTO(progressResult.value)
