...
import { type UserLearningPathProgressDTO, userLearningPathProgressMapper } from '../../infrastructure/learning-path.mapper'
...
export class StartLearningPathUseCase extends BaseUseCase<StartLearningPathRequest, UserLearningPathProgressDTO> {
  constructor(private learningPathRepository: ILearningPathRepository) {
    super()
  }

  async execute(request: StartLearningPathRequest): Promise<Result<UserLearningPathProgressDTO>> {
    const { learningPathId, userId } = request

    // Check if user has already started
    const hasStartedResult = await this.learningPathRepository.hasUserStarted(userId, learningPathId)
    ...
    if (hasStartedResult.value) {
      return Result.fail(new BusinessRuleError('User has already started this learning path'))
    }
    ...
    // Find START node
    const startNode = learningPath.nodes.find((node) => node.type === NodeType.START)

    if (!startNode) {
      return Result.fail(new BusinessRuleError('Learning path has no START node'))
    }
