...
export class LearningPathNode extends Entity<LearningPathNodeProps> {
   ...
   */
  static create(props: Omit<LearningPathNodeProps, 'id' | 'createdAt' | 'updatedAt'>): Result<LearningPathNode, ValidationError> {
    // Validations
    ...
    if (!Object.values(NodeType).includes(props.type)) {
      return Result.fail(new ValidationError('Invalid node type', 'type'))
    }

    // COURSE nodes must have a courseId
    if (props.type === NodeType.COURSE && !props.courseId) {
      return Result.fail(new ValidationError('Course nodes must have a courseId', 'courseId'))
    }

    // Non-COURSE nodes should not have a courseId
    if (props.type !== NodeType.COURSE && props.courseId) {
      return Result.fail(new ValidationError('Only course nodes can have a courseId', 'courseId'))
    }

    if (!props.position || typeof props.position.x !== 'number' || typeof props.position.y !== 'number') {
      return Result.fail(new ValidationError('Valid position is required', 'position'))
    }
