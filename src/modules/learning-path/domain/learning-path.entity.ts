...
import { LearningPathCreatedEvent, LearningPathPublishedEvent } from './events'
...
export class LearningPath extends AggregateRoot<LearningPathProps> {
   ...
   */
  static create(
    props: Omit<LearningPathProps, 'id' | 'isPublished' | 'nodes' | 'edges' | 'createdAt' | 'updatedAt'>
  ...
  publish(): Result<void, BusinessRuleError> {
    if (this._props.isPublished) {
      return Result.fail(new BusinessRuleError('Learning path is already published'))
    }
    ...
    if (!hasStartNode) {
      return Result.fail(new BusinessRuleError('Learning path must have a START node before publishing'))
    }

    // Validate that the learning path has at least one END node
    const hasEndNode = this.nodes.some((node) => node.type === 'END')

    if (!hasEndNode) {
      return Result.fail(new BusinessRuleError('Learning path must have an END node before publishing'))
    }
  ...
  unpublish(): Result<void, BusinessRuleError> {
    if (!this._props.isPublished) {
      return Result.fail(new BusinessRuleError('Learning path is not published'))
    }
  ...
  addNode(node: LearningPathNode): Result<void, BusinessRuleError> {
    ...
    if (nodeExists) {
      return Result.fail(new BusinessRuleError('Node already exists in learning path'))
    }
  ...
  removeNode(nodeId: string): Result<void, BusinessRuleError> {
    ...
    if (nodeIndex === -1) {
      return Result.fail(new BusinessRuleError('Node not found in learning path'))
    }

    // Remove all edges connected to this node
    if (this._props.edges) {
      this._props.edges = this._props.edges.filter((edge) => edge.sourceNodeId !== nodeId && edge.targetNodeId !== nodeId)
    }
  ...
  addEdge(edge: LearningPathEdge): Result<void, BusinessRuleError> {
    ...
    const sourceExists = this._props.nodes?.some((n) => n.id === edge.sourceNodeId)
    const targetExists = this._props.nodes?.some((n) => n.id === edge.targetNodeId)

    if (!sourceExists) {
      return Result.fail(new BusinessRuleError('Source node does not exist'))
    }

    if (!targetExists) {
      return Result.fail(new BusinessRuleError('Target node does not exist'))
    }

    // Check if edge already exists
    const edgeExists = this._props.edges.some((e) => e.sourceNodeId === edge.sourceNodeId && e.targetNodeId === edge.targetNodeId)

    if (edgeExists) {
      return Result.fail(new BusinessRuleError('Edge already exists in learning path'))
    }
  ...
  removeEdge(edgeId: string): Result<void, BusinessRuleError> {
    ...
    if (edgeIndex === -1) {
      return Result.fail(new BusinessRuleError('Edge not found in learning path'))
    }
