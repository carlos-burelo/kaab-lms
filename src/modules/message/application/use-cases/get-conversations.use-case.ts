...
export class GetConversationsUseCase extends BaseUseCase<GetConversationsRequest, GetConversationsResponse> {
  constructor(private conversationRepository: IConversationRepository) {
    super()
  }

  async execute(request: GetConversationsRequest): Promise<Result<GetConversationsResponse>> {
    const { currentUserId, limit, offset } = request

    // Find all conversations for user
    const conversationsResult = await this.conversationRepository.findByUser(currentUserId, {
      limit,
      offset
    })
    ...
    const totalUnreadResult = await this.conversationRepository.getTotalUnreadCount(currentUserId)
    ...
    // Map to DTOs
    const conversationDTOs = conversationsResult.value.map((c) => conversationMapper.toDTO(c))
