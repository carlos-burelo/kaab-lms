...
export class MarkMessagesAsReadUseCase extends BaseUseCase<MarkMessagesAsReadRequest, void> {
  constructor(
  ...
  async execute(request: MarkMessagesAsReadRequest): Promise<Result<void>> {
    ...
    const unreadMessagesResult = await this.messageRepository.findUnreadByConversationAndUser(conversationId, currentUserId)
    ...
    const markAsReadResult = await this.messageRepository.markManyAsRead(messageIds)
    ...
    const savedConversationResult = await this.conversationRepository.save(conversation)
