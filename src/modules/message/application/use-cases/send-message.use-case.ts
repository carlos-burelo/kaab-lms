...
export class SendMessageUseCase extends BaseUseCase<SendMessageRequest, MessageDTO> {
  constructor(
  ...
  async execute(request: SendMessageRequest): Promise<Result<MessageDTO>> {
    ...
    // Find conversation
    const conversationResult = await this.conversationRepository.findById(dto.conversationId)
