...
export class Conversation extends AggregateRoot<ConversationProps> {
  ...
  static create(
    props: Omit<
      ConversationProps,
      'id' | 'lastMessageId' | 'unreadCount1' | 'unreadCount2' | 'createdAt' | 'updatedAt'
    >
  ): Result<Conversation, ValidationError> {
    // Validations
    if (!props.participant1Id || props.participant1Id.trim().length === 0) {
      return Result.fail(
        new ValidationError('Participant 1 ID is required', 'participant1Id')
      );
    }

    if (!props.participant2Id || props.participant2Id.trim().length === 0) {
      return Result.fail(
        new ValidationError('Participant 2 ID is required', 'participant2Id')
      );
    }
  ...
  sendMessage(message: Message): Result<void, BusinessRuleError> {
    ...
    if (
      message.senderId !== this.participant1Id &&
      message.senderId !== this.participant2Id
    ) {
      return Result.fail(
        new BusinessRuleError('Sender must be a participant in the conversation')
      );
    }
  ...
  markMessagesAsRead(userId: string): Result<void, ValidationError> {
    if (userId !== this.participant1Id && userId !== this.participant2Id) {
      return Result.fail(
        new ValidationError('User is not a participant in this conversation', 'userId')
      );
    }
