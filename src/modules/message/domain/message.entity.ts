...
import { MessageReadEvent, MessageSentEvent } from './events'
...
export class Message extends AggregateRoot<MessageProps> {
  ...
  static create(
    ...
    if (!props.conversationId || props.conversationId.trim().length === 0) {
      return Result.fail(new ValidationError('Conversation ID is required', 'conversationId'))
    }

    if (!props.senderId || props.senderId.trim().length === 0) {
      return Result.fail(new ValidationError('Sender ID is required', 'senderId'))
    }
    ...
    if (props.content.trim().length > 10000) {
      return Result.fail(new ValidationError('Message content must not exceed 10000 characters', 'content'))
    }
  ...
  markAsRead(): Result<void, BusinessRuleError> {
    if (this._props.isRead) {
      return Result.fail(new BusinessRuleError('Message is already marked as read'))
    }
