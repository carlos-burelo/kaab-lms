...
export class Notification extends AggregateRoot<NotificationProps> {
  ...
  static create(
    ...
    if (!props.content || props.content.trim().length === 0) {
      return Result.fail(new ValidationError('Content is required', 'content'))
    }

    if (props.content.trim().length < 3) {
      return Result.fail(new ValidationError('Content must be at least 3 characters', 'content'))
    }
  ...
  markAsRead(): Result<void, BusinessRuleError> {
    if (this._props.isRead) {
      return Result.fail(new BusinessRuleError('Notification is already marked as read'))
    }
  ...
  markAsUnread(): Result<void, BusinessRuleError> {
    if (!this._props.isRead) {
      return Result.fail(new BusinessRuleError('Notification is already marked as unread'))
    }
