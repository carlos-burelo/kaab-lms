...
export class CompleteTaskUseCase extends BaseUseCase<CompleteTaskRequest, PersonalTaskDTO> {
  ...
  async execute(request: CompleteTaskRequest): Promise<Result<PersonalTaskDTO>> {
    ...
    if (!task.canBeEditedBy(currentUserId)) {
      return Result.fail(new ForbiddenError('You do not have permission to complete this task'))
    }
