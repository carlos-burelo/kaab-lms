...
export class GetEventUseCase extends BaseUseCase<GetEventRequest, CalendarEventDTO> {
  ...
  async execute(request: GetEventRequest): Promise<Result<CalendarEventDTO>> {
    ...
    if (!event.canBeEditedBy(currentUserId)) {
      return Result.fail(new ForbiddenError('You do not have permission to view this event'))
    }
