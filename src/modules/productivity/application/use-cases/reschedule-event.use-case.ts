...
export class RescheduleEventUseCase extends BaseUseCase<RescheduleEventRequest, CalendarEventDTO> {
  ...
  async execute(request: RescheduleEventRequest): Promise<Result<CalendarEventDTO>> {
    ...
    if (!event.canBeEditedBy(currentUserId)) {
      return Result.fail(new ForbiddenError('You do not have permission to reschedule this event'))
    }
