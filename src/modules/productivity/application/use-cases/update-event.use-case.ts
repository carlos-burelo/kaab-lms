...
export class UpdateEventUseCase extends BaseUseCase<UpdateEventRequest, CalendarEventDTO> {
  ...
  async execute(request: UpdateEventRequest): Promise<Result<CalendarEventDTO>> {
    ...
    if (!event.canBeEditedBy(currentUserId)) {
      return Result.fail(new ForbiddenError('You do not have permission to edit this event'))
    }
