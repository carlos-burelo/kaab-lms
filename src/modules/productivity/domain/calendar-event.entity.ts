...
import { EventType, getEventTypeColor } from './value-objects';
import {
  EventCreatedEvent,
  EventUpdatedEvent,
...
export class CalendarEvent extends AggregateRoot<CalendarEventProps> {
  ...
  static create(
    ...
    if (props.endDate <= props.startDate) {
      return Result.fail(
        new ValidationError('End date must be after start date', 'endDate')
      );
    }

    if (props.reminder && props.reminder < 0) {
      return Result.fail(
        new ValidationError('Reminder must be a positive number', 'reminder')
      );
    }
  ...
  conflicts(other: CalendarEvent): boolean {
    ...
    return (
      (this._props.startDate < other.endDate &&
        this._props.endDate > other.startDate)
    );
  }
