...
export class DeleteAnswerOptionUseCase extends BaseUseCase<
  DeleteAnswerOptionRequest,
  DeleteAnswerOptionResponse
> {
  async execute(request: DeleteAnswerOptionRequest): Promise<Result<DeleteAnswerOptionResponse>> {
    ...
    try {
      ...
      return Result.fail(
        new Error(`Failed to delete answer option: ${(error as Error).message}`)
      );
    }
  }
}
