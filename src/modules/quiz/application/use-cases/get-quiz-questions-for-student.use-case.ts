...
export class GetQuizQuestionsForStudentUseCase extends BaseUseCase<
  GetQuizQuestionsForStudentRequest,
...
> {
  async execute(request: GetQuizQuestionsForStudentRequest): Promise<Result<QuizQuestionsResponse>> {
    ...
    try {
      ...
      return Result.fail(
        new Error(`Failed to get quiz questions: ${(error as Error).message}`)
      );
    }
  }
}
