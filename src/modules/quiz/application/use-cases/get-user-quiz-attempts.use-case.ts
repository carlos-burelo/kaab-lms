...
export class GetUserQuizAttemptsUseCase extends BaseUseCase<
  GetUserQuizAttemptsRequest,
  QuizAttemptDTO[]
> {
  async execute(request: GetUserQuizAttemptsRequest): Promise<Result<QuizAttemptDTO[]>> {
    ...
    try {
      ...
      return Result.fail(
        new Error(`Failed to get user quiz attempts: ${(error as Error).message}`)
      );
    }
  }
}
