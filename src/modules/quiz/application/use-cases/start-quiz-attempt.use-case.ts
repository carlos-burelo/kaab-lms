...
export class StartQuizAttemptUseCase extends BaseUseCase<StartQuizAttemptRequest, QuizAttemptDTO> {
    ...
    try {
      ...
      if (quiz.maxAttempts) {
        ...
        if (attemptCount >= quiz.maxAttempts) {
          return Result.fail(new Error(`Maximum attempts (${quiz.maxAttempts}) reached`))
        }
      }
      ...
      return Result.fail(new Error(`Failed to start quiz attempt: ${(error as Error).message}`))
    }
  }
}
