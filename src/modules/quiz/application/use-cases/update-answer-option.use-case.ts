...
export class UpdateAnswerOptionUseCase extends BaseUseCase<UpdateAnswerOptionRequest, AnswerOptionDTO> {
  async execute(request: UpdateAnswerOptionRequest): Promise<Result<AnswerOptionDTO>> {
    ...
    try {
      ...
      return Result.fail(new Error(`Failed to update answer option: ${(error as Error).message}`))
    }
  }
}
