...
import { QuizCreatedEvent, QuizPublishedEvent } from './events'
...
export class Quiz extends AggregateRoot<QuizProps> {
   ...
   * Create a new quiz
   */
  static create(props: Omit<QuizProps, 'id' | 'isPublished' | 'createdAt' | 'updatedAt'>): Result<Quiz, ValidationError> {
    // Validations
    ...
    if (props.passingScore < 0 || props.passingScore > 100) {
      return Result.fail(new ValidationError('Passing score must be between 0 and 100', 'passingScore'))
    }

    if (props.durationMinutes && props.durationMinutes <= 0) {
      return Result.fail(new ValidationError('Duration must be positive', 'durationMinutes'))
    }

    if (props.maxAttempts && props.maxAttempts <= 0) {
      return Result.fail(new ValidationError('Max attempts must be positive', 'maxAttempts'))
    }
